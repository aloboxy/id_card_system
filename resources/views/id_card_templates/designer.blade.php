...
@push('styles')
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Alfa+Slab+One&family=Cinzel&family=EB+Garamond&family=Grand+Hotel&family=Inter&family=Montserrat&family=Open+Sans&family=Pacifico&family=Playfair+Display&family=Poppins&family=Roboto&family=Work+Sans&display=swap"
        rel="stylesheet">
    <style>
        /* Font Families */
        .font-helvetica {
...
@section('content')
    <div class="flex flex-col h-[calc(100vh-8rem)]">
                ...
                <div class="p-4 space-y-4 overflow-y-auto flex-1">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="text" id="template-name" value="{{ $template->name ?? 'New Template' }}"
                                    onchange="updateTemplateName(this.value)"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Active</label>
                                <input type="checkbox" name="is_active" id="template-active" {{ isset($template) && $template->is_active ? 'checked' : '' }} onchange="updateTemplateActive()"
                                    class="w-5 h-5">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">School</label>
                                <select id="school-select" onchange="updateSchool(this.value)"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                            ...
                            <div>
                                ...
                                <select id="role-select" onchange="updateRole(this.value)"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="number" id="template-width" value="{{ $template->width ?? 350 }}"
                                        onchange="updateCanvasSize()"
                                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                                </div>
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Height</label>
                                    <input type="number" id="template-height" value="{{ $template->height ?? 550 }}"
                                        onchange="updateCanvasSize()"
                                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <button onclick="addText()"
                            class="w-full flex items-center px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-md text-sm text-gray-700 transition-colors">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <button onclick="addRect()"
                                class="flex items-center justify-center px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-md text-sm text-gray-700 transition-colors">
                    ...
                    <div>
                        ...
                        <button onclick="document.getElementById('img-upload').click()"
                            class="w-full flex items-center px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-md text-sm text-gray-700 transition-colors mb-2">
                        ...
                        <input type="file" id="bg-upload" class="hidden" accept="image/*"
                            onchange="handleBackgroundUpload(this)">
                        <button onclick="removeBackground()"
                            class="w-full flex items-center px-3 py-2 mt-2 bg-red-50 hover:bg-red-100 rounded-md text-sm text-red-600 transition-colors">
                    ...
                    <div>
                        ...
                        <div id="student-placeholders"
                            class="{{ (isset($template) && $template->role == 'staff') ? 'hidden' : '' }}">
                            ...
                            <div class="space-y-2">
                                ...
                                <button type="button" onclick="addPlaceholder('class_with_section', 'Class & Section')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-graduation-cap w-5"></i> Class
                                </button>
                                <button onclick="addPlaceholder('dob', 'Date of Birth')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                        ...
                        <div id="staff-placeholders"
                            ...
                            <div class="space-y-2">
                                ...
                                <button onclick="addPlaceholder('designation', 'Designation')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-briefcase w-5"></i> Designation
                                </button>
                                <button onclick="addPlaceholder('department', 'Department')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-building w-5"></i> Department
                                </button>
                                <button onclick="addPlaceholder('joining_date', 'Joining Date')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-calendar-check w-5"></i> Joining Date
                                </button>
                                <button onclick="addPlaceholder('phone', 'Phone')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-phone w-5"></i> Phone
                                </button>
                                <button onclick="addPlaceholder('email', 'Email')"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                        ...
                        <div class="mb-3 mt-4">
                            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Photo Placeholder
                            </h3>
                            <div class="grid grid-cols-3 gap-2">
                                <button type="button" onclick="addPhotoPlaceholder('rectangle')"
                                    class="flex items-center justify-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-square"></i>
                                </button>
                                <button type="button" onclick="addPhotoPlaceholder('rounded')"
                                    class="flex items-center justify-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-square-full"></i>
                                </button>
                                <button type="button" onclick="addPhotoPlaceholder('circle')"
                                    class="flex items-center justify-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                        ...
                        <div class="mt-4">
                            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Other Elements
                            </h3>
                            <div class="space-y-2">
                                <button onclick="addIssueDate()"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                                    <i class="fas fa-calendar w-5"></i> Issue Date
                                </button>
                                <button onclick="addExpiryDate()"
                                    class="w-full flex items-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-md text-sm transition-colors">
                    ...
                    <div id="properties-panel" class="hidden border-t border-gray-200 pt-4 mt-4">
                        ...
                        <div class="space-y-3">
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Width</label>
                                    <input type="number" id="prop-width" onchange="updateDimension('width', this.value)"
                                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                                </div>
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">Height</label>
                                    <input type="number" id="prop-height" onchange="updateDimension('height', this.value)"
                                        class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Color</label>
                                <input type="color" id="prop-color" onchange="updateProperty('fill', this.value)"
                                    class="w-full h-8 rounded cursor-pointer">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Font Family</label>
                                <select id="prop-fontfamily" onchange="updateProperty('fontFamily', this.value)"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                                    <optgroup label="Modern / Graphic">
                                        <option value="Helvetica" style="font-family: 'Helvetica', 'Arial', sans-serif;">
                                            Helvetica</option>
                                        <option value="Futura" style="font-family: 'Futura', 'Trebuchet MS', sans-serif;">
                                            Futura</option>
                                        ...
                                        <option value="Avenir" style="font-family: 'Avenir', 'Nunito', sans-serif;">Avenir
                                        </option>
                                        <option value="Work Sans" style="font-family: 'Work Sans', sans-serif;">Work Sans
                                        </option>
                                    ...
                                    <optgroup label="Traditional / Serif">
                                        ...
                                        <option value="Baskerville" style="font-family: 'Baskerville', serif;">Baskerville
                                        </option>
                                        <option value="EB Garamond" style="font-family: 'EB Garamond', serif;">EB Garamond
                                        </option>
                                    </optgroup>

                                    <optgroup label="Styling / Display">
                                        <option value="Playfair Display" style="font-family: 'Playfair Display', serif;">
                                        ...
                                        <option value="Alfa Slab One" style="font-family: 'Alfa Slab One', cursive;">Alfa
                                            Slab One</option>
                                        <option value="Grand Hotel" style="font-family: 'Grand Hotel', cursive;">Grand Hotel
                                        </option>
                                    </optgroup>
                            ...
                            <div>
                                ...
                                <input type="number" id="prop-fontsize"
                                    onchange="updateProperty('fontSize', parseInt(this.value))"
                                    class="w-full px-2 py-1 text-sm border border-gray-300 rounded">
                            </div>
                            <div class="flex space-x-1">
                                <button onclick="toggleFontStyle('bold')" id="btn-bold"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200 font-bold"
                                    title="Bold">B</button>
                                <button onclick="toggleFontStyle('italic')" id="btn-italic"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200 italic"
                                    title="Italic">I</button>
                                <button onclick="toggleFontStyle('underline')" id="btn-underline"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200 underline"
                                    title="Underline">U</button>
                            </div>
                            <div class="flex space-x-1">
                                <button onclick="toggleTextAlign('left')" id="btn-align-left"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200"
                                    title="Align Left"><i class="fas fa-align-left"></i></button>
                                <button onclick="toggleTextAlign('center')" id="btn-align-center"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200"
                                    title="Align Center"><i class="fas fa-align-center"></i></button>
                                <button onclick="toggleTextAlign('right')" id="btn-align-right"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200"
                                    title="Align Right"><i class="fas fa-align-right"></i></button>
                                <button onclick="toggleTextAlign('justify')" id="btn-align-justify"
                                    class="flex-1 px-2 py-1 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded border border-gray-200"
                                    title="Justify"><i class="fas fa-align-justify"></i></button>
                            </div>
                            <button onclick="deleteSelected()"
                                class="w-full px-3 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-md text-sm transition-colors">
                                Delete Selected
                            </button>
                        </div>
                    </div>
            ...
            <div class="flex-1 bg-gray-100 flex flex-col relative overflow-hidden">
                ...
                <div class="bg-white border-b border-gray-200 p-2 flex justify-between items-center">
                    <div class="flex space-x-2">
                        <button onclick="setSide('front')" id="btn-front"
                            class="px-4 py-2 rounded-md text-sm font-medium bg-indigo-600 text-white">Front Side</button>
                        <button onclick="setSide('back')" id="btn-back"
                            class="px-4 py-2 rounded-md text-sm font-medium text-gray-600 hover:bg-gray-100">Back
                            Side</button>
                    </div>
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <input type="range" id="zoom-slider" min="50" max="200" value="100"
                                oninput="setZoom(this.value)" class="w-32">
        ...
        <form id="save-form"
            action="{{ (isset($template) && $template->exists) ? route('id-card-templates.update', $template) : route('id-card-templates.store') }}"
            ...
            <input type="hidden" name="active" id="hidden-active"
                value="{{ isset($template) && $template->is_active ? '1' : '0' }}">
